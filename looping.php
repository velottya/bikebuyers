...
for ($j = 1; $j <= 987; $j++) {
    ...
    $result = mysqli_query($connect, "SELECT income, commute_distance, age FROM data1 WHERE id = $j");
    $row = mysqli_fetch_assoc($result);
    $kolom1 = $row['income'];
    $kolom2 = $row['commute_distance'];
    $kolom3 = $row['age'];
    ...
    if (!empty($row['center1'])) {
        $Center1Class1 = $row['center1'];
    } else {
        $Center1Class1 = 0;
    }
    
    if (!empty($row['center2'])) {
        $Center2Class1 = $row['center2'];
    } else {
        $Center2Class1 = 0;
    }
    
    if (!empty($row['center3'])) {
    ...
    if (!empty($row['center1'])) {
        $Center1Class2 = $row['center1'];
    } else {
        $Center1Class2 = 0;
    }
    
    if (!empty($row['center2'])) {
        $Center2Class2 = $row['center2'];
    } else {
        $Center2Class2 = 0;
    }
    
    if (!empty($row['center3'])) {
        $Center3Class2 = $row['center3'];
    } else {
        $Center3Class2 = 0;
    }
    ...
    $jarak1 = sqrt(((int)$kolom1 - (int)$Center1Class1) ** 2 + ((int)$kolom2 - (int)$Center2Class1) ** 2 + ((int)$kolom3 - (int)$Center3Class1) ** 2);
    $jarak2 = sqrt(((int)$kolom1 - (int)$Center1Class2) ** 2 + ((int)$kolom2 - (int)$Center2Class2) ** 2 + ((int)$kolom3 - (int)$Center3Class2) ** 2);

    // Determine the closest cluster center and assign the data point to the corresponding class
    $minimum = min($jarak1, $jarak2);
    if ($minimum == $jarak1) {
        $out = 0;
    } else {
        $out = 1;
    ...
    mysqli_query($connect, "UPDATE data1 SET income = $kolom1, commute_distance = $kolom2, age = $kolom3, C1 = $jarak1, C2 = $jarak2, class = $out WHERE id = $j");
}
// Update cluster centers for each class
$result = mysqli_query($connect, "SELECT 
    (SELECT AVG(income) FROM data1 WHERE class = 0) as avgC1,
    (SELECT AVG(commute_distance) FROM data1 WHERE class = 0) as avgC2,
    (SELECT AVG(age) FROM data1 WHERE class = 0) as avgC3,
    (SELECT AVG(income) FROM data1 WHERE class = 1) as avgC4,
    (SELECT AVG(commute_distance) FROM data1 WHERE class = 1) as avgC5,
    (SELECT AVG(age) FROM data1 WHERE class = 1) as avgC6");
...
$purchased_bike = isset($_GET['purchased_bike']) ? (int)$_GET['purchased_bike'] : null;


$jarak1 = sqrt(($income - $avgC1) ** 2 + ($commute_distance - $avgC2) ** 2 + ($age - $avgC3) ** 2);
$jarak2 = sqrt(($income - $avgC4) ** 2 + ($commute_distance - $avgC5) ** 2 + ($age - $avgC6) ** 2);
...
echo "<script>";
if ($out == 0) {
    echo "alert('Tidak Jadi Beli');";
} else {
    echo "alert('Jadi Beli');";
}
echo "</script>";
...
mysqli_query($connect, "INSERT INTO data (income, commute_distance, age, purchased_bike, C1, C2, class, hasil) VALUES ($income, $commute_distance, $age, $purchased_bike, $jarak1, $jarak2, $out, $hout)");
...
<header>
    ...
    <div class="container">
        <div class="container pt-4">
            <br>
            <div class="card">
                <div class="card-header text-center"><b>Hasil K-Means dan Naive Bayes</b></div>
                <div class="m-3">
    ...
    <?php
    $no = 1;
    $cluster = mysqli_query($connect, "SELECT * FROM data");
